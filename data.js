 export const data = [
    { heading : 'ထီးယူသွားပါ☔️' ,
      text : 'Make sure to grab an umbrella before you head out' ,
      image : 'images/rain.gif'
    } ,
    { heading : 'အိမ်သော့မေ့မယ် သဲသဲ🔑' ,
      text : 'Remember to take your keys with you' ,
      image : 'images/key.gif'
    } ,
    { heading : 'အနွေးထည်ကောပါရဲ့လား🧥' ,
      text : 'Are you sure you have your jacket with you?' ,
      image : 'images/cold.gif'
    } ,
    { heading : 'လမ်းကူးရင်သေချာကူးပါ🚸' ,
      text : 'Be sure to check for traffic when you are crossing the street' ,
      image : 'images/road.gif'
    } ,
    { heading : 'Boltကိုဖြေးဖြေးမောင်းခိုင်းပါ🛵' ,
      text : 'Remember to tell the driver to go slowly when you take a Bolt motorbike' ,
      image : 'images/drive.gif'
    } ,
    { heading : 'Suncream မမေ့နဲ့နော်🧴' ,
      text : 'Make sure you pack some sunscreen' ,
      image : 'images/sun.gif'
    } ,
    { heading : 'အဆင်မငမ်းနဲ့👀' ,
      text : 'Don\'t go looking at other guys while you\'re out' ,
      image : 'images/look.gif'
    } ,
    { heading : 'အပြင်ရောက်ရင်စာပိုပါ💬' ,
      text : 'Don\'t forget to text me when you get outside' ,
      image : 'images/message.gif'
    } ,
    { heading : 'reportတင်ပါ🥹' ,
      text : 'Where are you off to? who are you going with?' ,
      image : 'images/report.gif'
    } ,
    { heading : 'မုန့်တွေအများကြီးမစားနဲ့🍟' ,
      text : 'Try to limit the street food today' ,
      image : 'images/snack.gif'
    } ,
    { heading : 'မီးတွေပိတ်သွားအုံး💡' ,
      text : 'Don\'t forget to turn off all the lights when you leave!' ,
      image : 'images/light.gif'
    } ,
    { heading : 'အဲကွန်းပိတ်သွားနော်❄️' ,
      text : 'Mind turning off the AC when you leave' ,
      image : 'images/ac.gif'
    } ,
    { heading : 'ဒေါက်ဖိနပ်မစီးသွားနဲ့နော်👡' ,
      text : 'Don\'t wear heels if you\'re going to be walking a lot' ,
      image : 'images/heel.gif'
    } ,
]